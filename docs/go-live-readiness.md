...
| Teacher workflow | Needs Work | Student requests, approval, dashboards, reports, and invite-code creation exist. Invite acceptance/revocation admin tools need completion. |
| Parent workflow | Needs Work | Student dashboard and progress summary exist. Multi-child parent account management needs a full child profile UI. |
...
| Area | Status | Notes |
| --- | --- | --- |
| Stripe-hosted payment | Completed | Payment Links are used for initial checkout/donations. Card data never touches the app. |
...
| Stripe webhook backend | Needs Work | Firebase Functions scaffold exists at `functions/src/index.ts`; it must be deployed and configured with Stripe secrets and price IDs. |
| Billing portal | Needs Work | Account UI supports portal links and guards against short-lived session URLs. Production should use the callable `createBillingPortalSession`. |
...
| Business metrics dashboard | Blocked | Needs backend aggregation for MRR, churn, active users, and feature usage. |
...
| Parent consent | Needs Work | Consent language exists; signup checkbox and consent record storage need completion. |
| Data deletion | Needs Work | Policy and support path exist; backend deletion/export workflow needs implementation. |
| Support operations | Needs Work | Support page/docs exist. Ticket routing, SLA, and status page need external setup. |
...
| Backend dependency audit | Blocked | `functions/` typechecks, but Firebase Admin/Functions currently pull moderate `uuid` advisories through Google Cloud dependencies. npm's force fix conflicts with Firebase peer dependencies. Recheck before deploying Functions. |
...
5. Confirm Family Plus and Teacher Pro docs appear in `subscriptions/{uid}` after checkout.
...
9. Complete parent consent checkbox and deletion request operations.
